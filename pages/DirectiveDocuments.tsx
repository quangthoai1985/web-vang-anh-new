...
const DirectiveDocuments: React.FC = () => {
  ...
  // Fetch Documents
  useEffect(() => {
    const q = query(collection(db, 'directive_documents'), orderBy('ngayBanHanh', 'desc'));
  ...
  const filteredDocs = useMemo(() => {
    let result = documents.filter(doc => {
      ...
      const matchesTab = activeTab === 'Tất cả' || doc.coQuanBanHanh === activeTab;
  ...
  const confirmDelete = async () => {
    if (docToDelete) {
      try {
        await deleteDoc(doc(db, 'directive_documents', docToDelete.id));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        const storageRef = ref(storage, `directive_documents/${Date.now()}_${selectedFile.name}`);
      ...
      if (isEditMode && formData.id) {
        await updateDoc(doc(db, 'directive_documents', formData.id), saveDocData);
        addToast("Cập nhật thành công", "Thông tin văn bản đã được lưu.", "success");
      } else {
        await addDoc(collection(db, 'directive_documents'), saveDocData);
  ...
  const getAuthorityBadgeColor = (auth: string) => {
    switch (auth) {
      case 'Bộ GD&ĐT': return 'bg-red-50 text-red-600 border-red-100';
      case 'Sở GD&ĐT': return 'bg-blue-50 text-blue-600 border-blue-100';
      case 'UBND Tỉnh': return 'bg-emerald-50 text-emerald-600 border-emerald-100';
  ...
  const getDocTypeIcon = (type: string) => {
    switch (type) {
      ...
      default: return <FileText className="h-5 w-5 text-red-500" />;
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] animate-in fade-in duration-500 pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between h-auto md:h-16 py-3 md:py-0 gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-lg md:text-xl font-bold text-gray-900 tracking-tight">VĂN BẢN CHỈ ĐẠO</h1>
              </div>
            </div>
            ...
            <div className="flex-1 max-w-2xl px-0 md:px-8 w-full">
              <div className="relative group">
                ...
                <input
                  type="text"
                  className="block w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl leading-5 placeholder-gray-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 sm:text-sm transition-all shadow-sm group-focus-within:shadow-md"
                  placeholder="Tìm kiếm văn bản, quyết định, thông báo..."
                  value={searchTerm}
            ...
            <div className="flex items-center justify-between md:justify-end w-full md:w-auto mt-2 md:mt-0 gap-3">
              <div className="flex items-center gap-2 no-scrollbar overflow-x-auto w-full md:w-auto">
                ...
                {['Tất cả', 'Bộ GD&ĐT', 'Sở GD&ĐT'].map(tab => (
                  <button
                    key={tab}
              ...
              {canManageDocuments && (
                <button
                  ...
                  className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl hover:bg-blue-700 transition-all shadow-md shadow-blue-200 active:scale-95 flex-shrink-0"
                >
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-800">Văn bản mới nhất</h2>
          <div
            className="flex items-center text-sm text-gray-500 cursor-pointer hover:text-blue-600 transition-colors select-none"
        ...
        {filteredDocs.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredDocs.map((doc) => (
              ...
              >
                {/* Decoration Gradient */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-gray-50 to-white rounded-bl-[100px] -z-0 opacity-50 group-hover:opacity-100 transition-opacity pointer-events-none"></div>

                {/* Card Header */}
                <div className="flex items-start justify-between mb-4 relative z-10">
                  <div className={`p-2.5 rounded-xl ${doc.fileDinhKemUrl && doc.fileDinhKemUrl.endsWith('.pdf') ? 'bg-red-50' : 'bg-blue-50'}`}>
                ...
                <div className="flex-1 relative z-10">
                  <h3 className="text-base font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors" title={doc.trichYeu}>
                    {doc.trichYeu}
                  </h3>
                  <p className="text-xs text-gray-500 mb-4 line-clamp-3 leading-relaxed">
                    {doc.tomTatNoiDung || "Chưa có tóm tắt nội dung. Bấm vào đề xem chi tiết văn bản đầy đủ."}
                  </p>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-50 relative z-10">
                  <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    <span className={`text-[10px] px-2 py-1 rounded-md font-medium ${getAuthorityBadgeColor(doc.coQuanBanHanh)}`}>
                      #{doc.coQuanBanHanh}
                    </span>
                  </div>
      ...
      {isViewModalOpen && selectedDoc && (
        <div className="fixed inset-0 z-[100] bg-gray-900 flex flex-col animate-in slide-in-from-bottom-5 duration-300">
          ...
          <div className="h-14 bg-[#1A1A1A] border-b border-gray-800 flex items-center justify-between px-4 lg:px-6 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs md:text-sm font-medium rounded-lg transition-colors"
              >
          ...
          <div className="flex-1 flex overflow-hidden relative">
            ...
            {/* Left Column: Preview (Interactive) - UPDATED: FULL SIZE */}
            <div className={`flex-1 bg-[#2C2C2C] relative flex flex-col transition-all duration-300 ${isMobile && viewMobileTab === 'info' ? 'hidden' : 'flex'}`}>
              {/* Minimal Toolbar for Preview Context */}
              ...
              {/* Full Iframe Container without padding/margins */}
            ...
            {/* Right Column: Info Sidebar (Collapsible on Mobile) - UPDATED: CLEANER */}
            <div className={`w-full md:w-[320px] lg:w-[360px] bg-white border-l border-gray-200 flex flex-col h-full overflow-y-auto ${isMobile ? (viewMobileTab === 'info' ? 'block' : 'hidden') : 'block'}`}>
              ...
              {/* Sidebar Content - Removed AI Summary, organized content */}
              <div className="p-5 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-blue-50 text-blue-700 text-xs">
                ...
                {/* Content Summary (if user provided) - Keeping it simple text */}
              ...
              <div className="mt-auto p-4 border-t border-gray-100 bg-gray-50">
                <button
                  onClick={() => handleDownload(selectedDoc)}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-200 hover:border-blue-400 text-gray-700 hover:text-blue-600 rounded-lg shadow-sm transition-all text-sm font-medium"
                >
            ...
            {isMobile && (
              <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex text-xs font-medium safe-area-inset-bottom z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                <button
                  ...
                  className={`flex-1 py-3 flex flex-col items-center justify-center gap-1 ${viewMobileTab === 'preview' ? 'text-blue-600 bg-blue-50/50' : 'text-gray-500'}`}
                >
                  <FileText className="h-5 w-5" />
                  <span>Văn bản</span>
                </button>
                <button
                  ...
                  className={`flex-1 py-3 flex flex-col items-center justify-center gap-1 ${viewMobileTab === 'info' ? 'text-blue-600 bg-blue-50/50' : 'text-gray-500'}`}
                >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              <form id="documentForm" onSubmit={handleSave} className="space-y-5">
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    required rows={3}
                    className="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-sm"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1">
                    ...
                    <select
                      className="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-sm bg-white"
                      value={formData.coQuanBanHanh || 'Bộ GD&ĐT'}
                      onChange={e => setFormData({ ...formData, coQuanBanHanh: e.target.value as any })}
                    >
                      {['Bộ GD&ĐT', 'Sở GD&ĐT', 'UBND Tỉnh', 'UBND Phường', 'Khác'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1">
                    ...
                    <select
                      className="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-sm bg-white"
                      value={formData.loaiVanBan || 'Công văn'}
                      onChange={e => setFormData({ ...formData, loaiVanBan: e.target.value as any })}
                    >
                      {['Công văn', 'Quyết định', 'Thông báo', 'Kế hoạch', 'Nghị quyết', 'Khác'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {selectedFileName ? (
                    <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-xl">
                      <div className="flex items-center gap-3">
                        <div className="bg-white p-2 rounded-lg text-blue-600"><FileText className="h-5 w-5" /></div>
                        <span className="text-sm font-medium text-blue-900 truncate max-w-[200px]">{selectedFileName}</span>
                      </div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center hover:bg-gray-50 hover:border-blue-400 transition-all cursor-pointer relative group">
                      ...
                      <p className="text-sm text-gray-500 group-hover:text-blue-600 font-medium">Bấm để chọn file hoặc kéo thả</p>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
              ...
              <button type="submit" form="documentForm" className="px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md shadow-blue-200 transition-all transform active:scale-95">
...
export default DirectiveDocuments;