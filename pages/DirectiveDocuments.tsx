...
const DirectiveDocuments: React.FC = () => {
  ...
  const handleOpenEdit = (doc: DirectiveDocument) => {
    ...
    setIsEditMode(true);
    setSelectedFile(null);
    // Simulate existing file if url exists (even if mock #) to show visual state
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        // If no file selected and no existing url, keep it as # or empty
        fileUrl = '#';
      }

      if (isEditMode && formData.id) {
        await updateDoc(doc(db, 'directive_documents', formData.id), {
          ...formData,
          fileDinhKemUrl: fileUrl
        });
        addToast("Cập nhật thành công", "Thông tin văn bản đã được lưu.", "success");
      } else {
        await addDoc(collection(db, 'directive_documents'), {
          ...formData,
          fileDinhKemUrl: fileUrl,
  ...
  const getAuthorityBadgeColor = (auth: string) => {
    switch (auth) {
      case 'Bộ GD&ĐT': return 'bg-red-100 text-red-700';
      case 'Sở GD&ĐT': return 'bg-blue-100 text-blue-700';
      case 'UBND Tỉnh': return 'bg-emerald-100 text-emerald-700';
        ...
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${isMobile ? 'py-4' : 'py-6'}`}>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              >
                ...
                <h1 className={`${isMobile ? 'text-lg' : 'text-2xl'} font-bold text-gray-900 uppercase`}>VĂN BẢN CHỈ ĐẠO</h1>
              </div>
            </div>
            {canManageDocuments && (
              <button
                onClick={handleOpenAdd}
                className={`flex items-center gap-2 bg-blue-600 text-white ${isMobile ? 'px-3 py-2 text-sm' : 'px-4 py-2'} rounded-lg hover:bg-blue-700 transition-colors shadow-md flex-shrink-0`}
              >
          ...
          <div className="mt-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex overflow-x-auto pb-1 lg:pb-0 gap-2 w-full lg:w-auto no-scrollbar">
              {['Tất cả', 'Bộ GD&ĐT', 'Sở GD&ĐT', 'UBND Tỉnh', 'UBND Phường'].map(tab => (
                <button
                  key={tab}
            ...
            <div className="flex w-full lg:w-auto gap-3">
              <div className="relative flex-grow lg:w-64 group">
                ...
                <input
                  type="text"
                  className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-shadow"
              ...
              {filteredDocs.length > 0 ? (
                filteredDocs.map((doc) => (
                  <div
                    ...
                    className="p-4 hover:bg-blue-50/50 transition-colors cursor-pointer active:bg-blue-100"
                  >
                    {/* Row 1: Title */}
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-2 ml-9">
                      ...
                      <Badge colorClass={getAuthorityBadgeColor(doc.coQuanBanHanh)}>{doc.coQuanBanHanh}</Badge>
                    ...
                    filteredDocs.map((doc) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-start gap-3">
                            <div className="p-2 rounded-lg bg-gray-100 text-gray-500 group-hover:bg-white group-hover:text-blue-600 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                              ...
                              <Badge colorClass="bg-yellow-100 text-yellow-800">{doc.loaiVanBan}</Badge>
                            </div>
                            <div className="flex items-center gap-4 text-xs text-gray-500">
                              <div className="flex items-center gap-1">
                                <Building2 className="h-3 w-3" />
                                <span className={getAuthorityBadgeColor(doc.coQuanBanHanh).replace('bg-', 'text-').split(' ')[1]}>
                                  {doc.coQuanBanHanh}
                                </span>
                              </div>
                              <div className="flex items-center gap-1">
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            {isMobile ? (
              <div className="h-full flex flex-col bg-gray-50">
                {/* Compact Header */}
                <div className="bg-blue-600 px-4 py-3 flex items-center gap-3 flex-shrink-0 safe-area-inset-top">
                  <button
                    onClick={() => setIsViewModalOpen(false)}
                    className="p-1.5 -ml-1.5 text-white/80 hover:text-white hover:bg-blue-700 rounded-full transition-all"
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-blue-100 text-[10px] font-mono">{selectedDoc.soKyHieu}</span>
                      <span className="text-blue-200 text-[10px]">•</span>
                      <span className="text-blue-100 text-[10px]">{new Date(selectedDoc.ngayBanHanh).toLocaleDateString('vi-VN')}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border-t border-gray-200 px-4 py-3 flex items-center justify-between gap-3 safe-area-inset-bottom">
                  <div className="flex items-center gap-2">
                    <Badge colorClass={getAuthorityBadgeColor(selectedDoc.coQuanBanHanh)}>{selectedDoc.coQuanBanHanh}</Badge>
                    <Badge colorClass="bg-yellow-100 text-yellow-800">{selectedDoc.loaiVanBan}</Badge>
                  </div>
                  <button
                    onClick={() => handleDownload(selectedDoc)}
                    className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg transition-all active:scale-95"
                  >
              ...
              <div className="h-full flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                  ...
                  {/* Modal Header */}
                  <div className="bg-blue-600 px-6 py-3 flex justify-between items-center flex-shrink-0 shadow-md z-20">
                    <h3 className="text-white font-semibold text-lg flex items-center gap-2">
                      <FileText className="h-5 w-5" /> Chi tiết văn bản
                    </h3>
                    <button onClick={() => setIsViewModalOpen(false)} className="text-blue-100 hover:text-white hover:bg-blue-700 p-1 rounded-full transition-all">
                      <X className="h-6 w-6" />
                    </button>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                    ...
                    <div className="w-full lg:w-[35%] flex flex-col border-r border-gray-200 bg-white h-full">
                      <div className="overflow-y-auto p-6 flex-1">
                        <div className="mb-6">
                          ...
                          <div className="flex flex-wrap gap-2 mt-3">
                            <Badge colorClass="bg-blue-100 text-blue-800">{selectedDoc.coQuanBanHanh}</Badge>
                            <Badge colorClass="bg-yellow-100 text-yellow-800">{selectedDoc.loaiVanBan}</Badge>
                      ...
                      <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center flex-shrink-0">
                        ...
                        <button
                          onClick={() => handleDownload(selectedDoc)}
                          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-md transition-colors"
                        >
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="p-6 overflow-y-auto">
                <form id="documentForm" onSubmit={handleSave} className="space-y-4">
                  ...
                  <div>
                    ...
                    {selectedFileName ? (
                      <div className="mt-1 p-4 border border-blue-200 rounded-lg bg-blue-50 flex items-center justify-between animate-in fade-in zoom-in duration-300">
                      ...
                      <div className="mt-1 relative flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 hover:bg-blue-50/30 transition-all cursor-pointer bg-gray-50 group">
                        <input
                          id="file-upload"
                          name="file-upload"
                          type="file"
                        ...
                        <div className="space-y-1 text-center pointer-events-none">
                          ...
                          <div className="flex text-sm text-gray-600 justify-center">
                            <span className="relative rounded-md font-medium text-blue-600 focus-within:outline-none group-hover:underline">
              ...
              <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3 flex-shrink-0">
                ...
                <button
                  type="submit"
                  form="documentForm"
                  className="flex items-center gap-2 px-6 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-md hover:shadow-lg transition-all"
                >
