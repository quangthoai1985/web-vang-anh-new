...
const OfficeDocuments: React.FC = () => {
   ...
   return (
      <div className="flex flex-col h-screen bg-gray-50 overflow-hidden font-sans">
         {/* --- Header --- */}
         <div className={`bg-white border-b border-teal-100 ${isMobile ? 'px-4 py-3' : 'px-6 py-4'} flex-shrink-0 z-20 shadow-sm`}>
            <div className="max-w-7xl mx-auto w-full">
               ...
               <div className="flex justify-between items-center gap-3">
                  <h1
                     ...
                     onClick={() => navigate('/')}
                  >
                     <span className={`${isMobile ? 'p-1.5' : 'p-2'} bg-teal-100 rounded-lg text-teal-700`}>
                        <FileText className={`${isMobile ? 'h-5 w-5' : 'h-6 w-6'}`} />
                     </span>
                     {isMobile ? 'Kho Kế hoạch & Báo cáo' : 'Kho Kế hoạch & Báo cáo'}
                  </h1>
                  {(user?.permissions?.includes('manage_documents') || user?.permissions?.includes('manage_office_docs')) && (
                     <button
                        onClick={handleOpenUpload}
                        className={`flex items-center gap-2 bg-teal-600 hover:bg-teal-700 text-white ${isMobile ? 'px-3 py-2 text-sm' : 'px-4 py-2'} rounded-lg shadow-md transition-colors font-medium`}
                     >
                        <Upload className="h-4 w-4" />
                        <span>{isMobile ? 'Tải lên kế hoạch mới' : 'Tải lên kế hoạch mới'}</span>
                     </button>
                  )}
               </div>
            </div>
         ...
         <div className="flex flex-1 overflow-hidden max-w-7xl mx-auto w-full">
            ...
            <div className={`flex-1 bg-gray-50 overflow-y-auto ${isMobile ? 'p-4' : 'p-6'}`}>
               ...
               {isMobile && (
                  <div className="flex gap-2 overflow-x-auto pb-3 mb-4 no-scrollbar">
                     {[
                        ...
                        { id: 'general', label: 'Hành chính' }
                     ].map(tab => (
                        <button
                           key={tab.id}
                           onClick={() => setActiveCategory(tab.id as any)}
                           className={`flex-shrink-0 px-3 py-1.5 text-xs font-bold rounded-full transition-all ${activeCategory === tab.id
               ...
               {/* Search Bar */}
               <div className={`${isMobile ? 'mb-4' : 'mb-6'} flex items-center bg-white border border-gray-200 rounded-lg px-3 py-2 ${isMobile ? '' : 'max-w-md'} shadow-sm focus-within:ring-2 focus-within:ring-teal-500 transition-all`}>
               ...
               {isMobile ? (
                  <div className="space-y-3">
                     {filteredDocs.map((docItem) => (
                        <div
                           key={docItem.id}
                           onClick={() => handleRowClick(docItem)}
                           className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md hover:border-teal-300 transition-all cursor-pointer"
                        >
                           <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                 <h4 className="text-sm font-bold text-gray-900 truncate">{docItem.name}</h4>
                                 <p className="text-xs text-gray-500 mt-0.5">{getCategoryLabel(docItem.category)}</p>
                              </div>
                           </div>
                           <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                              <div className="flex items-center gap-2">
                                 <div className="h-6 w-6 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-[10px]">
                                    {docItem.uploader.name.charAt(0)}
                                 </div>
                                 <span className="text-xs text-gray-600">{docItem.uploader.name}</span>
                                 <span className="text-[10px] text-gray-400">• {new Date(docItem.uploadDate).toLocaleDateString('vi-VN')}</span>
                              </div>
                              ...
                              >
                                 <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center">
                                       ...
                                       <div className="ml-4">
                                          <div className="text-sm font-medium text-gray-900 group-hover:text-teal-700 transition-colors">{docItem.name}</div>
                                          <div className="text-xs text-gray-500">{getCategoryLabel(docItem.category)}</div>
                                       </div>
                                    </div>
                                 </td>
                                 <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="flex items-center">
                                       ...
                                       <div className="ml-3">
                                          <div className="text-sm font-medium text-gray-900">{docItem.uploader.name}</div>
                                          <div className="text-xs text-gray-500">{docItem.uploader.role}</div>
                                       </div>
                                    </div>
                                 </td>
                                 ...
                                 <td className="px-6 py-4 whitespace-nowrap text-center">
                                    {(docItem.comments || []).length > 0 ? (
                                       <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 animate-pulse">
                                          ...
                                          {(docItem.comments || []).length} Góp ý mới
                                       </span>
                                    ) : (
            ...
            isMobile ? (
               ...
               <div className="fixed inset-0 z-50 bg-white flex flex-col animate-in fade-in duration-200">
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white flex-shrink-0 safe-area-inset-top">
                     ...
                     <div className="flex-1 min-w-0">
                        <h2 className="text-sm font-bold text-gray-900 truncate">{currentDoc.name}</h2>
                        <p className="text-[10px] text-gray-500">{getCategoryLabel(currentDoc.category)} • {new Date(currentDoc.uploadDate).toLocaleDateString('vi-VN')}</p>
                     </div>
                  </div>
                  ...
                  <div className="flex-shrink-0 bg-white border-t border-gray-200 px-4 py-3 safe-area-inset-bottom">
                     <div className="flex items-center gap-3">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                           <div className="h-7 w-7 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-[10px] flex-shrink-0">
                              {currentDoc.uploader.name.charAt(0)}
                           </div>
                           <span className="text-xs text-gray-600 truncate">{currentDoc.uploader.name}</span>
                        ...
                        <a
                           ...
                           download
                           className="flex items-center gap-1.5 px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium shadow-md"
